...
const Profile = () => {
    ...
    useEffect(() => {
        async function eventsFunction() {
            ...
            setEventArray(
                localEvents
                    .sort(function (a, b) {
                        return a.date_occurring - b.date_occurring;
                    })
                    .slice(0, 2)
            );
        }
    ...
    useEffect(() => {
        function renderAllEvents() {
            ...
            let arr = eventArray.map((event, index) => (
                <EventBlock
                    title={event.title}
                    description={event.description}
                    location={event.location}
                    spaces_total={event.spaces_total}
                    spaces_remaining={event.spaces_remaining}
                    date_occurring={event.date_occurring}
                    key={index}
                />
            ));
            setEventList(arr);
